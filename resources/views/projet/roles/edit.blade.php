...
<h3> Modifier Role: {{ $role->name }}</h3>

<form class="form-card" method="POST" action="{{ route('roles.update',$role->id) }}">
    @csrf
    @method('PUT')
    ...
    <div class="grid">
        @foreach($permissions as $permission)
            <label class="form-check-label" class="card">
    ...
    <button class="btn bg-success text-white" type="submit">💾 Save</button>
</form>
