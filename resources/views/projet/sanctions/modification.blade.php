...
<h3>Modifier Sanction</h3>

<form action="{{ route('sanctions.update', $sanction->id) }}" method="POST">
    @csrf
    @method('PUT')

    <input type="text" name="type" value="{{ $sanction->type }}" class="form-control mb-2">

    <input type="date" name="date" value="{{ $sanction->date }}" class="form-control mb-2">

    <textarea name="description" class="form-control mb-2">{{ $sanction->description }}</textarea>

    <select name="id_acteur" class="form-control mb-2">
        @foreach($acteurs as $acteur)
            <option value="{{ $acteur->id_acteur }}"
                {{ $acteur->id_acteur == $sanction->id_acteur ? 'selected' : '' }}>
                {{ $acteur->nom_prenom }}
            </option>
        @endforeach
    </select>
